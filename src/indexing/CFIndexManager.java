...
public class CFIndexManager {
	...
	protected int saveCodeFragmentIndex(int exceptionID,
		...
		int completed = 0;
		try {
			String fragmentFolder = StaticData.Surf_Data_Base
					+ "/fragmentsIndex/" + exceptionID;
	...
	protected void saveCodeFragments(int exceptionID,
		...
		for (CodeFragment fragment : Fragments) {
			String fileID = StaticData.Surf_Data_Base + "/fragments/"
					+ exceptionID + "/" + fragment.FragmentID + ".txt";
		...
		try {
			...
			if (fDir.isDirectory()) {
				for (File f : fDir.listFiles()) {
					...
					CodeFragment cfragment = (CodeFragment) oistream
							.readObject();
				...
				System.out.println("Exception ID:" + exceptionID
						+ " Fragments recovered:" + Fragments.size());
			}
		} catch (Exception exc) {
			// handle the exception
			exc.printStackTrace();
	...
	public static ArrayList<CodeFragment> readAllFragments() {
		ArrayList<CodeFragment> Fragments = new ArrayList<>();
		try {
			String folderPath = StaticData.Surf_Data_Base
					+ "/fragmentsIndexAll";
			File fDir = new File(folderPath);
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 150; i++) {
			...
			int exceptionID = i;
			String exceptionName = ExceptionKeyLoader
